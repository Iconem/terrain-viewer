...
// MapLayers.tsx's PhongRasterLayer), draped over 3D terrain the exact same
// automatic way the raster basemap already is. See lib/matcap-protocol.ts's
// header for why this is a plain raster tile rather than a custom WebGL
// layer with its own mesh.
//
// Light direction is compass-fixed (state.illuminationDir/illuminationAlt —
// the same fields the on-map "hold L, drag" light control and, formerly,
// maplibre's own hillshade illumination direction used), not camera-
// relative: panning/rotating the map must not spin the light, the way it's
// deliberately allowed to for a matcap material (see lib/matcap-protocol.ts).
// The raw normal encoding is already compass-aligned (x=east, y=south, z=up
...
const PHONG_URL_RE = /^phong:\/\/(-?[\d.]+)\/(-?[\d.]+)\/(-?[\d.]+)\/(-?[\d.]+)\/(terrarium|mapbox)\/(\d+)\/([^/]+)\/(\d+)\/(-?\d+)\/(-?\d+)$/

export function buildPhongProtocolUrl(
  diffuseStrength: number, specularStrength: number, lightDir: number, lightAlt: number,
  ...
  return base.replace("phong://", `phong://${diffuseStrength}/${specularStrength}/${lightDir}/${lightAlt}/`)
}
...
export async function phongProtocol(
  ...
  const [, diffuseStr, specularStr, lightDirStr, lightAltStr, encodingRaw, tileSizeStr, encodedTemplate, zStr, xStr, yStr] = match
