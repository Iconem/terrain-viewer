...
- `scripts/build-url-params.mjs` scans `components/TerrainViewer.tsx` for the nuqs parsers, the atom files for `atomWithStorage`, and writes `src/generated/url-params.json` and `public/openapi.json`. Both outputs are tracked in git, and a pre-commit hook (`simple-git-hooks`, configured in the root `package.json`, installed by `pnpm install`) regenerates and stages them, so a commit that changes a parameter carries the matching spec. `pnpm run url-params` runs it by hand.
