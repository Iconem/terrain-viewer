...
This page covers the mechanism — how a `*-protocol.ts` file gets from an upstream raster-dem tile to a rendered layer. For what each mode *means* geomorphologically, see [Terrain Visualization Modes](/features/visualization-modes); for the exact formulas, see [Equations & Formulas](/features/equations).
...
<p className="text-sm text-fd-muted-foreground italic">Slope + Hillshade backdrop — Matterhorn massif</p>
...
The exact per-mode formulas are on the [Equations page](/features/equations); this page is about what happens to the result once it's computed.
