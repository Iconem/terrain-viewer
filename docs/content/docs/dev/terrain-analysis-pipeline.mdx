...
Those packed bytes are then handed to maplibre as an **`ImageBitmap`**, via the shared `toTileImage` tail in `lib/tile-image.ts` — *not* PNG-encoded. That choice is worth three orders of magnitude per tile and is the subject of [the last section on this page](#handing-the-tile-back-to-maplibre).
...
**Register through `lib/protocol-registry.ts`, never `maplibregl.addProtocol` directly.** MapLibre only asks a protocol for the tiles of its own sources; every consumer in the app that builds a tile URL from a template and fetches it itself (each derived mode reading its upstream DEM, the GeoTIFF export, the 2D elevation picker and profile) goes through `fetchTileBitmap` / `dispatchTile` from that registry, which looks the scheme up and calls the handler. Before the registry each consumer kept its own list of schemes to special-case, and twice a new source type slipped through it: hillshade worked (MapLibre's own fetch) while every other mode drew nothing, first for `lerc://` and `quantized-mesh://`, then for `vrt://` and the nDSM's `demdiff://`. A scheme that is not registered now throws an error naming the registry instead of the browser's "URL scheme is not supported", and a consumer that goes through the registry never has to know which protocols exist. The GeoTIFF export, the 2D elevation picker and profile, and the contours layer all resolve a source through the same client upstream the viz modes read (`useClientDemUpstream`), so a source that renders in one mode exports, samples and contours in the others: VRT, LERC, quantized mesh, difference and WMS sources included. Contours over one of our schemes run maplibre-contour on the main thread with a `getTile` that dispatches through the registry, since its worker can only `fetch()`.
...
It simply was not advertised: the types said `ArrayBuffer` and the documented example showed a canvas. maplibre-gl-js [#8515](https://github.com/maplibre/maplibre-gl-js/issues/8515) and [#8525](https://github.com/maplibre/maplibre-gl-js/pull/8525) documented and typed the return shape after this was found; the timings are in [Tile Caches](/dev/tile-caches#handing-tiles-to-maplibre).
