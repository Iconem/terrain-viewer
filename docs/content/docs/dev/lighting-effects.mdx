...
The live-GL-layer version instead reconstructs a real per-fragment view ray from `gl_FragCoord` + FOV, reflects it off the (rotated) normal, and uses the reflection's `xy * 0.5 + 0.5` — a perspective-correct matcap, with an optional "light relative to camera" mode that unprojects through the inverse per-tile projection matrix.
