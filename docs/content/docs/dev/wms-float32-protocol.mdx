...
The RGBA buffer goes through the shared `toTileImage` tail (`lib/tile-image.ts`) and is returned to MapLibre as an `ImageBitmap`, decoded rather than PNG-encoded — worth roughly 99 ms a tile; see [Tile Caches](/dev/tile-caches#handing-tiles-to-maplibre).
