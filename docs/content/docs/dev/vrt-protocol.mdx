...
The `<SRS>` element carries the projection as WKT and `proj4` — already a dependency — turns that into a transform. The inverse runs **per output pixel**, 65 536 of them for a 256 tile, measured at 35-50 ms. That is the honest cost of doing this client-side and it is well below the source reads.
