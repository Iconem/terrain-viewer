...
The ion token is the one that gates a *terrain* source rather than a basemap: every Cesium ion asset answers 401 without it, so [Cesium World Terrain](/dev/quantized-mesh-protocol) and any private ion asset stay unreachable until it is set. A free token is at [ion.cesium.com/tokens](https://ion.cesium.com/tokens).
