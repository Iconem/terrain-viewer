...
Load them from **Sources → Terrain → Sample**, which opens a picker where each dataset is added or removed with one click (or all at once), or open any of them directly with a `?sourceA=<id>` link.
