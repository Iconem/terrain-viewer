...
import { NationalDatasetsTable, NationalCoverageMap, UnusableDatasetsTable, MapterhornSummary, Glo30Table, SubNationalTable } from "@/components/national-datasets-table";
...
Load them from **Sources → Terrain → Load Sample Sources**, or open any of them directly with a `?sourceA=<id>` link.
...
Some of the best data on this page covers a state or a province rather than a country. These are kept out of **Load Sample Sources** so that list stays national, but each is a normal source and loads from a `?sourceA=<id>` link.
...
**No agency anywhere publishes Terrarium tiles.** Only two publish a pre-rendered elevation tile pyramid at all (Japan's GSI and Mexico's INEGI), and both invented their own encoding. Everything else serves raw rasters on demand.
...
- **XYZ tiles** — a normal tile pyramid. Spain is standard Terrain-RGB. Mexico is not: its layer is *named* "terranium" but packs elevation with Terrain-RGB's factors and a `baseShift` of 1000 instead of 10000, so it is configured with [custom RGB encoding](/features/byod) and reads ~9 km too low without it.
