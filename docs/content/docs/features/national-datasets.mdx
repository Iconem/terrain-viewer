...
Load them from **Sources → Terrain → Library**, which opens a picker where each dataset is added or removed with one click (or all at once), graded against Mapterhorn and filterable by name, or open any of them directly with a `?terrainSourceA=<id>` link. To see where each one has data before loading it, turn on its footprint in [Coverage overlays](/features/coverage-overlays).
