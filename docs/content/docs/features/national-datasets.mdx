...
Load them from **Sources → Terrain → Library**, which opens a picker where each dataset is added or removed with one click (or all at once), graded against Mapterhorn and filterable by name, or open any of them directly with a `?sourceA=<id>` link.
...
Some of the best data on this page covers a state or a province rather than a country. These sit below the divider in the **Sample** picker and are left out of *Load all* so the default list stays national, but each is one click away there and loads from a `?sourceA=<id>` link.
