...
![OSM 3D buildings from OpenFreeMap Liberty, here over a Vantor post-event overlay in Nepal](/screenshots/osm-liberty-3d.jpg)
...
<p className="text-sm text-fd-muted-foreground italic">Basemap comparison, Side grid mode</p>
...
<p className="text-sm text-fd-muted-foreground italic">Historical Imagery, Split Mode Side (4×2) — ESRI Wayback and Google Earth Historical mixed across capture dates — Île de la Cité, Paris — see <a href="/features/split-modes#side">Split &amp; Compare Modes</a> for how the grid itself works</p>

### The timeline panel

![Scrubbing the historical timeline across capture dates](/screenshots/historical-timeline.jpg)

<p className="text-sm text-fd-muted-foreground italic">Historical timeline panel — scrubbing capture dates across providers</p>
...
```mermaid
flowchart LR
  D["Pick a date on\nthe timeline"] --> W["ESRI Wayback:\nsnap to nearest\nreal-capture RELEASE"]
  D --> G["Google Earth Historical:\nwalk the tile's own\nquadtree packet"]
  D --> H["HLS / Planet:\ndirect date → monthly\nmosaic URL"]
  W --> T["Tile rendered"]
  G --> T
  H --> T
