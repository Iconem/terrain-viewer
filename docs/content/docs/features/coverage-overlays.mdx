...
**The one thing about this layer that is not obvious, and that cost a week:** Google serves it *sharded across zoom levels*. Every zoom, coarse or fine, carries a *different* ~25 % subset of the polygons, and the fine zooms never converge — measured on the central‑Paris tile, z5 through z13 each cover 23–30 % alone, and the union climbs steadily with every level added (three zooms 48 %, six 72 %, nine 81 %). It behaves like sampling with replacement at about a quarter per level. So the lever is *more zoom levels*, not better clipping: the current file unions z5 through z12, which is what put London and San Francisco inside — they were never gaps in Google's coverage, they were shards on levels not yet fetched.

Three stages, in this order:

1. **Fetch** — `docs/scripts/google3d-coverage.ts fetch`, one hierarchical crawl from z5 down to z12: an empty tile is a fixed 36‑byte stub, so only non‑empty children are descended, and every intermediate level is kept on disk because every level is a different shard. 116 304 requests, 194 MB, 259 s, into `.cache/google3d-z12/` (1 024 / 376 / 844 / 1 876 / 4 624 / 11 024 / 26 336 / 70 200 tiles for z5…z12).
2. **Decode** — `google3d-coverage.ts decode --zoom N`, once per level out of that same directory: XOR, protobuf, triangles → rings by cancelling shared edges. Two things that earlier decoders got wrong and that silently delete cities: zero‑area collinear triangles are part of the mesh and must be counted, and vertices must keep their ~0.7 % overshoot rather than be clamped to the tile box. Coordinates go from the 8 192 grid to lng/lat, so the only quantisation at this point is the tile's own (4.8 m at z10, 1.2 m at z12). Output `.cache/google3d-z{5…12}.geojson` — 3 785 / 7 447 / 12 440 / 17 937 / 24 137 / 31 243 / 48 283 / 96 011 polygons; the deeper levels are the files to load in kepler.gl to see the raw shards. The decoder was audited stage by stage on the central‑Paris tile: triangles 24.9 % of the tile, rings 25.2 %, polygons 24.9 %, no duplicates, no unread geometry — it loses nothing; the tile really carries a quarter.
3. **Dissolve** — `docs/scripts/dissolve-google-3d-coverage.mjs`, all eight levels at once: snap every ring to 3 decimals (~110 m) and clean it, drop anything under 1 000 m²; bucket by 5°×5°; union each bucket in chunks of 400, thin each partial (125 m), then tree‑merge the partials pairwise until nothing merges; then per merged polygon drop holes under 1 km², simplify at ~500 m, truncate to 3 decimals, re‑node by unioning the polygon with itself, drop small holes again, drop outer rings under 1 km². 241 283 polygons → 9 333, 3.96 MB, 546 s with a 12 GB heap. Output `public/coverage/google-3d.geojson`.

Why that order matters is written up in the script's header: the clipper (`polygon-clipping`, under `@turf/turf`) throws on full‑precision input, an early fallback returned *unmerged halves* so three zooms shipped stacked, and the merge loop later gave up on a pass that could not pair anything. Checked: 20/20 on a city checklist; the central‑Paris tile at 74 % (48 % with three zooms); the wider Paris window 2 258 km² with one overlapping pair.
...
C:\Users\jonathan\.t3\worktrees\elevation-terrain-visualizer\t3code-619d1237\public\coverage\google-3d.geojson (shipped, 4.0 MB)
```

`pnpm google-3d-fetch` (needs `GOOGLE_KEY`) then `pnpm google-3d-dissolve` reproduces it, about fifteen minutes end to end. Going deeper than z12 would recover more (the curve has not flattened), at roughly four times the requests per extra level. Superseded but kept: `build-google-3d-coverage.mjs` classifies tiles by *response size* and needs no key (~39 km cells — the fallback if the layer id minting ever stops), `build-google-3d-coverage.py` is a shapely port of the decoder, and `decode-google-3d-coverage.mjs` is the earlier hand‑rolled decoder with the two bugs above.
