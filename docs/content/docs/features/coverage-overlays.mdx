...
- **FLAI open LiDAR** — open point clouds republished as COPC at [hub.flai.ai](https://hub.flai.ai/), one rectangle per survey. Not a DEM, but the same question: is there anything better than global 30 m here? Drawn hollow because each rectangle is a declared extent — a national survey's box clips its neighbours.
...
| FLAI open LiDAR | One rectangle per dataset | Each survey's COPC extent, read from the LAS header of its overview file with a 512-byte range request, by `docs/scripts/build-flai-coverage.mjs` into `public/coverage/flai-open-lidar.geojson` |
