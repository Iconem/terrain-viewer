...
- **Export any visible layer.** The DEM GeoTIFF button gets a menu: next to the elevation, **Visible layers…** exports each Terrain Analysis and Relief Visualization mode on screen, plus the contour lines, as its raw values (slope in degrees, LRM in metres...) in a float32 GeoTIFF, straight from the tiles already cached for the view. The DEM GeoTIFF itself was broken until now: the library writing it stored one byte per value, and GDAL could not open the file.
...
- VRT mosaics: a range read that times out or drops is retried once, and a tile that still fails is reported as failed instead of drawn with a hole. The hole used to stay cached for the session, which left blank bands in derived modes and straight "0 m" contour edges that never healed. A source file that is really gone still reads as a hole.
