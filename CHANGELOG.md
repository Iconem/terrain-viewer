...
- **Google 3D coverage: Google serves it sharded across zoom levels, and the file now unions eight of them.** Every zoom, fine ones included, carries a *different* quarter of the polygons and they never converge — on the central‑Paris tile z5–z13 each cover 23–30 % alone and the union climbs with every level added. The shipped file unions z5–z12 (was z8–z10): central Paris 48 % → 74 %, the wider window 1 333 → 2 258 km², the world +67 %, and London and San Francisco are inside — they were shards on unfetched levels, not gaps. Along the way the union stage was rebuilt: polygon-clipping throws on raw coordinates and the old fallback returned unmerged halves, so three zooms had shipped stacked as a crosshatch; and the merge loop was giving up on a pass that could not pair anything. 3.96 MB.
