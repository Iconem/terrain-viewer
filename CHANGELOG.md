...
- **URL keys**: the terrain source of a view is now `terrainSourceA`…`terrainSourceH` in the address bar, matching `basemapSourceA`…; old `sourceA=` links and saved bookmarks still work. New one-shot parameters `sidebar=open|closed` and `scrollTo=<section>`; `addOverlayUrl=` registers overlays and `overlayBasemapIds=` may name URLs directly. The Share dialog offers an iframe snippet with a hide-side-panel switch.
- Docs: the Embedding page now has a builder that can start from a pasted link, the full generated reference of state parameters and stored settings, and the project preset interface rendered from source.
