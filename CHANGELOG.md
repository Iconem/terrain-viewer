...
- GEDTM30 no longer crashes the map: a footprint spanning the full 360° of longitude reached MapLibre's max-bounds through the map's own prop and threw (reproduced on the served bundle); every max-bounds value is sanitised now. The difference source writes holes where either side has no data, and the hypsometric tint paints the DEM floor transparent.
