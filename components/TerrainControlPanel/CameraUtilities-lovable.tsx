/**
 * CameraUtilities.tsx — Refactored
 *
 * State ownership:
 ...
 *
 * RAF loops use module-level engines (animation-engine.ts) so they survive
 * sidebar close/reopen — the component just reads isRunning() on mount and
 * can stop via engine.stop().
 */
...
export function CameraButtons({ mapRef, appState, setAppState, setAppStateSafe }: CameraButtonsProps) {
  ...
  const [animParams, setAnimParams] = useQueryStates({
    ...
    animPose2:        parseAsSnapshot.withDefault(null as any),
  }, { shallow: true })
  ...
  const setPose1 = (v: AppSnapshot | null) => setAnimParams({ animPose1: v as any })
  const setPose2 = (v: AppSnapshot | null) => setAnimParams({ animPose2: v as any })
