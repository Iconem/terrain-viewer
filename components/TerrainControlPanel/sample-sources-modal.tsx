...
function kindOf(name: string): { label: string; title: string } | null {
  ...
  if (/nDSM|DSM − DTM|height above ground|elevation change|\(dh\)/i.test(name)) return { label: "nDSM", title: "nDSM, normalised digital surface model: DSM − DTM, the height of what stands on the ground (a CHM, canopy height model, over forest). Also used here for elevation-change grids: 0 is the ground, or no change" }
