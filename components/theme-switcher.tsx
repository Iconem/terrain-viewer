...
export const SOURCE_GROUPS: { key: NonNullable<ThemeConfig["source"]>; label: string }[] = [
  ...
  { key: "shadcnstudio", label: "shadcnstudio.com" },
...
// Themes shown in the leading "Default" group of the picker (the app's own
// baseline + the HeritageWatch AI brand theme), rather than in a source bucket.
export const DEFAULT_GROUP_SOURCES = new Set(["hwai"]);
...
export function ColorThemeSelect() {
  ...
  // The leading "Default" group: the app baseline plus any theme sourced as a
  // Default-group brand theme (HeritageWatch AI), rather than a source bucket.
  const defaultGroupThemes = useMemo(
    () => [defaultTheme, ...sortedThemes.slice(1).filter((t) => t.source && DEFAULT_GROUP_SOURCES.has(t.source))],
    [defaultTheme],
  );

  // Built-in presets (excluding the Default-group themes above) bucketed by
  // source site, each rendered as its own SelectGroup — order within a bucket
  // stays as sortedThemes already sorted it. A theme whose source has no bucket
  // (the Default-group brand themes) is skipped here, not crashed on.
  const bySource = useMemo(() => {
    const buckets = new Map<string, ThemeConfig[]>(SOURCE_GROUPS.map((g) => [g.key, []]));
    for (const t of sortedThemes.slice(1)) {
      const bucket = buckets.get(t.source ?? "tweakcn");
      if (bucket) bucket.push(t);
    }
    return buckets;
  }, []);

  // Cycling order matches the dropdown's own visual order: Default group first,
  // then saved custom themes, then grouped built-ins (themux, shadcnstudio,
  // tweakcn, shadcnthemes).
  const orderedNames = useMemo(
    () => [...defaultGroupThemes.map((t) => t.name), ...customThemes.map((t) => t.name), ...SOURCE_GROUPS.flatMap((g) => bySource.get(g.key)!.map((t) => t.name))],
